...
| 6 | `data-connectors/bigquery/bigqueryConnectorView.tsx` | Log export to BigQuery | config shipped in spec 008; runtime adapter tracked as spec 018 | config surface | real admin form | **`config in spec 008; runtime deferred to spec 018`** | BigQuery needs the cloud.google.com/go/bigquery SDK + streaming-insert auth plumbing — carved out as its own spec rather than bundled with Datadog. |
| 7 | `data-connectors/datadog/datadogConnectorView.tsx` | Log export to Datadog | config shipped in spec 008; runtime shipped in spec 017 | full | real admin form | **`shipped in specs 008 + 017`** | plugins/logexport is an ObservabilityPlugin that POSTs every completed trace to Datadog's /api/v2/logs intake, TTL-cached connector list with Invalidate() on CRUD, fail-open on network errors. |
