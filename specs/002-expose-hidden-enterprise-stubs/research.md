...
| 8 | `guardrails/guardrailsConfigurationView.tsx` | Central guardrail policies | **none** — no `guardrails_central` plugin | **none** | — | ContactUsView teaser | **`descope → FeatureStatusPanel`** | US6 net-new plugin. Upstream governance plugin has no guardrail-policy storage; per-workspace guardrail rules exist but as chain configs inside governance, not central policies. |
| 9 | `guardrails/guardrailsProviderView.tsx` | External guardrail provider credentials (Aporia, Patronus, etc.) | **none** | **none** | — | ContactUsView teaser | **`descope → FeatureStatusPanel`** | Partner-provider adapter pattern (FR-015) is US6 net-new. |
...
| 11 | `login/loginView.tsx` | Enterprise SSO login page (SAML/OIDC) | **none** — no `handlers/sso.go` | **none** | — | ContactUsView teaser | **`descope → FeatureStatusPanel`** | US3 net-new. Upstream `oauth2.go` handles per-user OAuth *for MCP clients*, not for operator login. |
...
| 20 | `user-rankings/userRankingsTab.tsx` | Top-users dashboard tab | **none** — `UserRankingsResponse` type is forward-declared in `ui/lib/types/logs.ts` but no `/api/logs/user-rankings` endpoint; `/api/logs/rankings` returns model-rankings only | **none** | type exists, hook doesn't | ContactUsView teaser | **`descope → FeatureStatusPanel`** | US12 descoped in spec 001 (T066). Per-user aggregation endpoint needs its own spec. |
