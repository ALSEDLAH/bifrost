...
- [X] T015 [P] [US2] ~~Replace stub body in `ui/app/enterprise/components/login/loginView.tsx` (title "Enterprise SSO Login", status "needs-own-spec", trackingLink → SR-01 row for US3).~~ **Superseded 2026-04-20**: flipped from FeatureStatusPanel to fallback re-export. The enterprise stub + `hideAuthDashboard = IS_ENTERPRISE` in securityView were together leaving the enterprise UI with no working auth UI at all. Reuse the OSS basic-auth login form (already complete upstream) until SSO ships under its own spec.
...
- [X] T017 [P] [US2] ~~Replace stub body in `ui/app/enterprise/components/orgs-workspaces/organizationSettingsView.tsx`~~ — **REMOVED 2026-04-20**: file was dead code (zero importers, no mounted route); deleted along with `_fallbacks` sibling instead of flipping to panel.
- [X] T018 [P] [US2] ~~Replace stub body in `ui/app/enterprise/components/orgs-workspaces/workspacesView.tsx`~~ — **REMOVED 2026-04-20**: same as T017; deleted both enterprise + `_fallbacks` copies. Canonical paths (`/workspace/governance/business-units`, `/workspace/governance/teams`) are the real features and were never routed through these files.
